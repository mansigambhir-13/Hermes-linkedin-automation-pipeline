"""WhatsApp variant of the v6 green "knowledge card".

The v6 green card was ORIGINALLY designed as the WhatsApp knowledge-card layout
(eyebrow masthead -> full-width course image inside a rounded dark card ->
rainbow headline -> brand-voice body with names bolded white -> centred Rehearsal
wordmark -> green "Download the Rehearsal app" button). Here it is rendered for
WhatsApp with WhatsApp-NATIVE captions:

  - WhatsApp markdown:  *bold*  _italic_  (single asterisks/underscores)
  - a "Forward this..." line (WhatsApp's native virality mechanic)
  - plain auto-linking URLs (iOS / Android / Web) — no hashtag spam
  - a copy-paste-ready .md per card (you post these manually; Postiz has no
    WhatsApp channel)

Card format: portrait 1080x1350 (4:5) — large in chat, fits WhatsApp Status,
Broadcast, and Channel posts. Same factual card data as LinkedIn + Instagram
(shared ../_cards_v6.py) so definitions never drift between platforms.

Usage:  python3 _build.py                 # all 60 cards
        python3 _build.py first-principles keeper-test
"""
...
HERE = Path(__file__).parent              # postizz/glossary/whatsapp
...
FORWARD = "_Forward this to someone prepping for placements._"
...
bolden = _meta.bolden                      # HTML <b> bolder for the CARD
...
def wa_bold(text, terms):
    """WhatsApp *bold* on key names/companies (longest first; skip if already wrapped)."""
...
def wa_caption(c):
    """WhatsApp-native caption, copy-paste ready."""
    terms = BOLD.get(c["slug"], [])
    body = "\n\n".join(wa_bold(p, terms) for p in paras(c))
    return (
        f"*{c['headline']}*\n\n"
        f"{body}\n\n"
        f"{c['close']}\n\n"
        f"{FORWARD}\n\n"
        f"📲 Download the Rehearsal app\n{LINKS}"
    )
...
# --- card HTML (portrait 1080x1350, identical green knowledge-card) --------
...
MD = """# {headline} — WhatsApp (Business Glossary)

**Platform:** WhatsApp (Status / Broadcast / Channel) · portrait card `{slug}.png` (1080x1350)
**Series:** The Business Glossary
**Voice:** Rehearsal calm-authoritative documentary observer (social-media-pillars-2026-05)
**Course:** `{course}`
**Image:** `visuals/{course}/{image}`
**Posting:** manual — attach `{slug}.png`, paste the caption below (WhatsApp keeps the *bold* / _italic_ markdown).
...
def main(targets):
    ...
    for slug in slugs:
        ...
        if not img.exists():
            print(f"  ! MISSING image: {img}", file=sys.stderr); continue
        body_html = "\n      ".join(
            f'<div class="def">{bolden(p, BOLD.get(slug, []))}</div>' for p in paras(c)[:3]
        )
        html = (HTML.replace("__IMAGE__", str(img)).replace("__GRAD__", GRADIENT)
                .replace("__HEADLINE__", c["headline"]).replace("__BODY__", body_html)
                .replace("__HSIZE__", str(hsize(c["headline"]))))
        html_out = html_dir / f"{slug}.html"
        png_out = HERE / f"{slug}.png"
        html_out.write_text(html)
        (HERE / f"{slug}.md").write_text(MD.format(
            headline=c["headline"], slug=slug, course=c["course"],
            image=c["image"], caption=wa_caption(c),
        ))
        render(html_out, png_out)
        print(f"  WA {slug:24s} {c['course']}/{c['image']}  -> {png_out.name}")
    print(f"\n  done ({len(slugs)} cards)")
