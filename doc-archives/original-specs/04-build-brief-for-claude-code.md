...
### LinkedIn
- OAuth 2.0; post via the LinkedIn API with media. One-click after approval is achievable early.
- Confirm the org/page vs personal posting target and required scopes during setup.
...
1. **Locked config values** — the exact CTA style + per-platform hashtag sets the team wants appended. (Provide these; the agent must not invent them.)
...
6. **Image model API access** — which providers the team can get keys for (affects the default per-job routing).
